...
# キューを初期化 (10秒間のデータを保持するため、20個のデータを格納できるキューを用意)
queue_length = 2
temperature_queue = deque([43] * queue_length, maxlen = queue_length)
error_queue = deque([6] * queue_length, maxlen=queue_length)  # Initialize error deque with zero

# PID parameters
set_temp = 50.0  # Set temperature
Kp = 10.5  #13.2         # Propotinal gain
Ki = 0.0175  #0.12       # Integral gain
Kd = 125.0 #363      # differential gain

###### set for CSV ######
# CSVファイルの保存先ディレクトリ
csv_dir = '/Users/shingo/Documents/Temperature_Controller/picosdk-python-wrappers/usbtc08Examples/Temp Control/Data_Multi_TimeAve_dtLong'
csv_filename = os.path.join(csv_dir, '10sec_Kp10R5_Ki0R0175_Kd125.csv')
...
integral = 0
# error_queue = deque([6] * queue_length, maxlen=queue_length)  # Initialize error deque with zero
sample_time = 0.5  # Sampling time: 0.5 seconds for 4 channels

# PID control logic
def calculate_pid(temp):
    global integral, error_queue
    error = set_temp - temp
    error_queue.append(error)  # Append current error to the deque
    integral += error * sample_time

    # Calculate derivative using the first and the last error in the queue
    derivative = (error_queue[-1] - error_queue[0]) / (sample_time * (len(error_queue) - 1))

    output = Kp * error + Ki * integral + Kd * derivative
    output = round(output, 2)  # Round output to 2 decimal places
    output = max(0, min(output, 73))  # Clamp output to voltage limits of PMX70-1A (0 to 73.5V)
...
try:
    while True:
        ...
        volt, integral_value = calculate_pid(avg_temp_5_sec)  # temp1 is at index 0
